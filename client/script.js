    ...
    newListButton.addEventListener('click', function() {
      ...
      function createNewList(listName) {
        ...
        function deleteList(listName) {
          sortResult = []
            fetch(`/api/favorite-lists/${encodeURIComponent(listName)}`, {
                method: 'DELETE',
            })
            .then(response => {
                if (!response.ok) {
    ...
    function searchSuperheroes(query, field, n) {
      ...
      .then(ids => {
          const fetchPromises = ids.map(id => {
              const superheroPromise = fetch(`/api/superheroes/${id}`).then(response => response.json());
              const powersPromise = fetch(`/api/superheroes/${id}/powers`).then(response => response.json());
